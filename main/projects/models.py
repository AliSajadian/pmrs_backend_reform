...
class Invoice(models.Model):
    ...
    def confirmedInvoiceFCAmounts(self):
        """
        Get the confirmed invoice amounts for the invoice.
        """
        return (self.aci_g_fc or 0) + (self.aca_g_fc or 0) + (self.ew_g_fc or 0)
    ...
    def sentInvoiceFCAmounts(self):
        """
        Get the sent invoice amounts for the invoice.
        """
        return (self.icc_g_fc or 0) + (self.acc_g_fc or 0) + (self.ewcc_g_fc or 0)
    ...
    def allFCReceived(self):
        """
        Get the all received for the invoice.
        """
        a = ((self.ccpi_a_vat_ew_fc or 0) - (self.cvat_fc or 0))
        return a
    ...
    def confirmedFCAmount(self):
        """
        Get the confirmed fc amount for the invoice.
        """
        b = ((self.aci_n_fc or 0) + (self.aca_n_fc or 0) + (self.ew_n_fc or 0)) 
        return b 
    ...
    def totalCumulativeReceiveFCAmount(self):
        """
        Get the total cumulative receive fc amount for the invoice.
        """
        financialInfos = FinancialInfo.objects.filter(contractid__exact=self.contractid, dateid__exact=self.dateid)
        financialInfo = financialInfos[0] if financialInfos and len(financialInfos) > 0 else None
        lastclaimbill = financialInfo.lastclaimbill_fc if financialInfo else 0
        
        return (self.ccpi_a_vat_ew_fc or 0) + (lastclaimbill or 0)
